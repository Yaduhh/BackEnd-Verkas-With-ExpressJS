...
const getAllCategories = async (req, res, next) => {
  try {
    ...
    if (type && type !== 'undefined' && type !== '') {
      // For transactions, we filter t.type, but categories no longer have type.
      // If master is filtering categories by type, we should ignore it or return nothing.
      // Since categories are flexible, filtering them by type doesn't make sense anymore.
    }
    ...
    if (type && type !== 'undefined' && type !== '') {
      // Ignore type filter for categories
    }
    ...
    const limitNum = toSafeInt(limit, 50);
    const offsetNum = (pageNum - 1) * limitNum;
    sql += ` ORDER BY c.name ASC LIMIT ${limitNum} OFFSET ${offsetNum}`;
