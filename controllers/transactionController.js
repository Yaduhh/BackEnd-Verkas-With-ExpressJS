...
const getAll = async (req, res, next) => {
  try {
    ...
    const transactions = await Transaction.findAll({
      ...
      paymentMethodCategoryId: payment_method_category_id || undefined,
      includeIncomeDetails: include_income_details === 'true'
    });
...
const update = async (req, res, next) => {
  try {
    ...
    if (income_details !== undefined) {
      const oldI = JSON.stringify(existing.income_details || []);
      const newI = JSON.stringify(transaction.income_details || []);
      if (oldI !== newI) {
        changes.income_details = { old: existing.income_details, new: transaction.income_details };
        changedFields.push('Rincian Pemasukan');
      }
    }
