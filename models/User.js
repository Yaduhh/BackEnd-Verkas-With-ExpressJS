...
class User {
  ...
  static async findById(id) {
    const results = await query(
      'SELECT id, email, name, role, created_by, created_at, updated_at, web_session_token FROM users WHERE id = ? AND status_deleted = false',
      [id]
    );
  ...
  // Update web session token
  static async updateWebSessionToken(id, token) {
    await query(
      'UPDATE users SET web_session_token = ? WHERE id = ? AND status_deleted = false',
      [token, id]
    );
    return true;
  }
